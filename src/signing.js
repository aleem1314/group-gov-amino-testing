...
import { groupAminoConverter } from "./types/group/amino";
...
import {
  MsgDeposit,
  MsgSubmitProposal,
  MsgVote as GovMsgVote,
} from "./proto/ts/cosmos/gov/v1/tx";
import { govv1AminoConverter } from "./types/gov/amino";

const canUseAmino = () => {
  return true;
};

const getKeplrClient = async (chainId, messages) => {
  let signer;
  if (!canUseAmino()) {
    try {
...
export const signAndBroadcast = async (
  ...
  let signer;
  try {
    signer = await getKeplrClient(chainId, messages);
  } catch (error) {
    console.log(error);
    throw new Error("failed to get keplr", error);
  }
  ...
  registry.register("/cosmos.gov.v1.MsgVote", GovMsgVote);
...
async function simulate(
  ...
  const fee = getFee(50_000, gasPrice);
  const txBody = {
...
function convertToAmino(aminoTypes, messages) {
...
async function sign(
  ...
  let aminoMsgs;
  try {
    aminoMsgs = convertToAmino(aminoTypes, messages);
  } catch (error) {
    console.log(error);
  }
